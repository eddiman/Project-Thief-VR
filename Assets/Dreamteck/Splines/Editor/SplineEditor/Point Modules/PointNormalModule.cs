namespace Dreamteck.Splines.Editor
{
    ...
    public class PointNormalModule : PointModule
    {
            ...
            for (int i = 0; i < selectedPoints.Count; i++)
            {
                switch (mode)
                {
                    ...
                    case 11:
                        ...
                        points[selectedPoints[i]].normal = Vector3.Cross(result.direction, result.right).normalized;
                        break;
                }
            }
        }
        ...
        void AutoNormal(int index)
        {
            ...
            Handles.DrawWireDisc(evalResult.position, evalResult.direction, HandleUtility.GetHandleSize(points[index].position) * 0.5f);
            Handles.color = color;
