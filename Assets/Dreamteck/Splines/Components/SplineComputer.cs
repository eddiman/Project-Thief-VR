namespace Dreamteck.Splines
{
    ...
    public partial class SplineComputer : MonoBehaviour
    {
        ...
        List<SplineComputer> junctionUpdateIgnore = new List<SplineComputer>();
        ...
        public void WriteSamples(SampleCollection collection)
        {
        ...
        /// <returns></returns>
        public virtual void Project(SplineSample result, Vector3 position, double from = 0.0, double to = 1.0, EvaluateMode mode = EvaluateMode.Cached, int subdivisions = 4)
        {
            if(mode == EvaluateMode.Calculate)
            {
        ...
        public virtual SplineSample Project(Vector3 point, double from = 0.0, double to = 1.0)
        {
            SplineSample result = new SplineSample();
        ...
        private void TransformSamples(bool forceTransformAll = false)
        {
            ...
            if (_sampleMode == SampleMode.Optimized)
            {
                ...
                for (int i = 0; i < sampleCollection.optimizedIndices.Length; i++)
                {
                    double samplePercent = (double)i / (sampleCollection.optimizedIndices.Length - 1);
                }
        ...
        void OptimizeSamples()
        {
            ...
            _sampleCount = 2;
            Vector3 lastSavedDirection = _transformedSamples[0].direction;
            sampleFlter[0]  = true;
            ...
            {
                float angle = Vector3.Angle(lastSavedDirection, _transformedSamples[i].direction);
                if (angle >= _optimizeAngleThreshold)
                {
                    sampleFlter[i] = true;
                    _sampleCount++;
                    lastSavedDirection = _transformedSamples[i].direction;
                }
        ...
        public void ResetTriggers()
        {
            ...
            {
                for (int j = 0; j < triggerGroups[i].triggers.Length; j++)
                {
                    triggerGroups[i].triggers[j].Reset();
                }
            }
        }
