...
namespace Dreamteck.Splines
{
    ...
    public class WaveformGenerator : MeshGenerator
    {
        ...
        private void Generate()
        {
            ...
            {
                Vector3 samplePosition = GetSample(i).position;
                Vector3 localSamplePosition = spline.InverseTransformPoint(samplePosition);
                Vector3 bottomPosition = localSamplePosition;
                Vector3 sampleDirection = GetSample(i).direction;
                Vector3 sampleNormal = GetSample(i).normal;

                float heightPercent = 1f;
                if (_uvWrapMode == UVWrapMode.UniformX || _uvWrapMode == UVWrapMode.Uniform)
                {
                    if (i > 0) totalLength += Vector3.Distance(GetSample(i).position, GetSample(i - 1).position);
                }
                switch (_axis)
                {
                ...
                for (int n = 0; n < _slices + 1; n++)
                {
                    ...
                    switch (_uvWrapMode)
                    {
                        case UVWrapMode.Clamp: tsMesh.uv[vertIndex] = new Vector2((float)GetSample(i).percent * uvScale.x + uvOffset.x, slicePercent * uvScale.y + uvOffset.y); break;
                        ...
                        case UVWrapMode.UniformY: tsMesh.uv[vertIndex] = new Vector2((float)GetSample(i).percent * uvScale.x + uvOffset.x, heightPercent * slicePercent * uvScale.y + uvOffset.y); break;
                    ...
                    tsMesh.colors[vertIndex] = GetSample(i).color * color;
                    vertIndex++;
                }
            }
