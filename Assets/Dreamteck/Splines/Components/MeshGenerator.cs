...
namespace Dreamteck.Splines
{
    public class MeshGenerator : SplineUser
    {
        ...
        {
            vDist = 0f;
            if (uvMode == UVMode.UniformClip) vDist = spline.CalculateLength(0.0, GetSample(0).percent);
        }

        protected void AddUVDistance(int sampleIndex)
        {
            if (sampleIndex == 0) return;
            vDist += Vector3.Distance(GetSample(sampleIndex).position, GetSample(sampleIndex - 1).position);
        }
