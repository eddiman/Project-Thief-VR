    ...
    public class PathGenerator : MeshGenerator
    {
        ...
        void GenerateVertices()
        {
            ...
            for (int i = 0; i < sampleCount; i++)
            {
                ...
                Quaternion rot = Quaternion.AngleAxis(rotation, evalResult.direction);
                ...
                for (int n = 0; n < _slices + 1; n++)
                {
                    ...
                    tsMesh.vertices[vertexIndex] = center + rot * right * fullSize * 0.5f - rot * right * fullSize * slicePercent + rot * evalResult.normal * shapeEval * _shapeExposure;
                    ...
                    if (_slices > 1)
                    {
                        if (n < _slices)
                        {
                            ...
                            Vector3 nextVertPos = center + rot * right * fullSize * 0.5f - rot * right * fullSize * forwardPercent + rot * evalResult.normal * shapeEval * _shapeExposure;
                            Vector3 cross1 = -Vector3.Cross(evalResult.direction, nextVertPos - tsMesh.vertices[vertexIndex]).normalized;

                            if (n > 0)
                            {
                                Vector3 cross2 = -Vector3.Cross(evalResult.direction, tsMesh.vertices[vertexIndex] - lastVertPos).normalized;
                                tsMesh.normals[vertexIndex] = Vector3.Slerp(cross1, cross2, 0.5f);
                            } else tsMesh.normals[vertexIndex] = cross1;
                        }
                        else   tsMesh.normals[vertexIndex] = -Vector3.Cross(evalResult.direction, tsMesh.vertices[vertexIndex] - lastVertPos).normalized;
                    }
                    else
                    {
                        tsMesh.normals[vertexIndex] = evalResult.normal;
