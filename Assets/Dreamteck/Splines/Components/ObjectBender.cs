...
namespace Dreamteck.Splines
{
    ...
    public class ObjectBender : SplineUser
    {
        ...
        private void GetevalResult(Vector3 percentage)
        {
            ...
            switch (_normalMode)
            {
                case NormalMode.Auto: evalResult.normal = Vector3.Cross(evalResult.direction, evalResult.right); break;
                case NormalMode.Custom: evalResult.normal = _customNormal; break;
            }
            if (_forwardMode == ForwardMode.Custom) evalResult.direction = customForward;
            ...
            switch (axis)
            {
                case Axis.Z:
                    ...
                    evalResult.position += evalResult.normal * Mathf.Lerp(bounds.min.y, bounds.max.y, percentage.y) * evalResult.size;
                    break;
                case Axis.X:
                ...
                case Axis.Y:
                    ...
                    evalResult.position += right * Mathf.Lerp(bounds.min.x, bounds.max.x, percentage.x) * evalResult.size;
                    evalResult.position += evalResult.normal * Mathf.Lerp(bounds.min.z, bounds.max.z, percentage.z) * evalResult.size;
                    break;
            }
        ...
        public void BendObject(BendProperty p)
        {
            ...
            if (p.originalSpline != null)
            {
                for (int n = 0; n < p.splinePointPercents.Length; n++)
                {
                    ...
                    switch (axis)
                    {
                        case Axis.X: point.normal = Quaternion.LookRotation(evalResult.direction, evalResult.normal) * Quaternion.FromToRotation(Vector3.up, evalResult.normal) * point.normal; break;
                        case Axis.Y: point.normal = Quaternion.LookRotation(evalResult.direction, evalResult.normal) * Quaternion.FromToRotation(Vector3.up, evalResult.normal) * point.normal; break;
                        case Axis.Z: point.normal = Quaternion.LookRotation(evalResult.direction, evalResult.normal) * point.normal; break;
                    }
