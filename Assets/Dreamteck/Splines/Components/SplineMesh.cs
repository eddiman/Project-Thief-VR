    ...
    public class SplineMesh : MeshGenerator
    {
        ...
        void Generate()
        {
            ...
            for (int i = 0; i < channels.Count; i++)
            {
                ...
                {
                    float avgBounds = 0f;
                    for (int j = 0; j < channels[i].GetMeshCount(); j++) avgBounds += channels[i].GetMesh(j).bounds.size.z;
        ...
        private void Place(Channel channel, TS_Mesh target, double percent)
        {
            ...
            Vector3 originalNormal = evalResult.normal;
            Vector3 originalRight = evalResult.right;
            Vector3 originalDirection = evalResult.direction;
            if (channel.overrideNormal)
            {
                evalResult.direction = Vector3.Cross(evalResult.right, channel.customNormal);
                evalResult.normal = channel.customNormal;
            }
            Quaternion axisRotation = Quaternion.identity;
            vertexMatrix.SetTRS(evalResult.position + originalRight * (offset.x + channeloffset.x) * evalResult.size + originalNormal * (offset.y + channeloffset.y) * evalResult.size + originalDirection * offset.z, evalResult.rotation * channel.NextPlaceRotation(percent) * Quaternion.AngleAxis(rotation, Vector3.forward), channel.NextPlaceScale() * evalResult.size);
            ...
            for (int i = 0; i < Mathf.Min(target.colors.Length, definition.colors.Length); i++) target.colors[i] = definition.colors[i] * evalResult.color;
        }
        ...
        private void Stretch(Channel channel, TS_Mesh target, double from, double to)
        {
            ...
            for (int i = 0; i < definition.vertexGroups.Count; i++)
            {
                ...
                Vector3 originalNormal = modifiedResult.normal;
                Vector3 originalRight = modifiedResult.right;
                Vector3 originalDirection = modifiedResult.direction;
                if (channel.overrideNormal)
                {
                    modifiedResult.direction = Vector3.Cross(modifiedResult.right, channel.customNormal);
                    modifiedResult.normal = channel.customNormal;
                }
                ...
                vertexMatrix.SetTRS(modifiedResult.position + originalRight * (offset.x + channelOffset.x) * modifiedResult.size + originalNormal * (offset.y + channelOffset.y) * modifiedResult.size + originalDirection * offset.z, 
                    modifiedResult.rotation * Quaternion.AngleAxis(rotation + channelRotation, Vector3.forward), 
                    channelScale * modifiedResult.size);
                normalMatrix = vertexMatrix.inverse.transpose;
