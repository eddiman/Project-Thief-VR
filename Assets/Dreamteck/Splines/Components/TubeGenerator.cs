    ...
    public class TubeGenerator : MeshGenerator
    {
        ...
        void Generate()
        {
            ...
            for (int i = 0; i < sampleCount; i++)
            {
                ...
                if (offset != Vector3.zero) center += offset.x * right + offset.y * evalResult.normal + offset.z * evalResult.direction;
                ...
                for (int n = 0; n < _sides + 1; n++)
                {
                    ...
                    Quaternion rot = Quaternion.AngleAxis(_revolve * anglePercent + rotation + 180f, evalResult.direction);
                    ...
                    tsMesh.colors[vertexIndex] = evalResult.color * color;
                    vertexIndex++;
                }
            }
        ...
        void GenerateFlatCaps()
        {
            ...
            for (int i = 0; i < _sides+1; i++)
            {
                ...
                tsMesh.normals[index] = -GetSample(0).direction;
            ...
            for (int i = 0; i < _sides + 1; i++)
            {
                ...
                tsMesh.normals[index] = GetSample(sampleCount -1).direction;
        ...
        {
            //Start Cap
            Vector3 center = GetSample(0).position;
            Quaternion lookRot = Quaternion.LookRotation(-GetSample(0).direction, GetSample(0).normal);
            float startV = 0f;
                        float capLengthPercent = 0f;
            switch (uvMode)
            {
                case UVMode.Clip: startV = (float)GetSample(0).percent;
                    capLengthPercent = (size * 0.5f) / spline.CalculateLength(); break;
                case UVMode.UniformClip:
                    startV = spline.CalculateLength(0.0, GetSample(0).percent);
            ...
            for (int lat = 1; lat < _roundCapLatitude+1; lat++)
            {
                ...
                for (int lon = 0; lon <= sides; lon++)
                {
                    ...
                    tsMesh.vertices[index] = center + lookRot * rot * -Vector3.right * size * 0.5f * GetSample(0).size;
                    tsMesh.colors[index] = GetSample(0).color * color;
            ...
            //End Cap
            center = GetSample(sampleCount - 1).position;
            lookRot = Quaternion.LookRotation(GetSample(sampleCount - 1).direction, GetSample(sampleCount - 1).normal);
            switch (uvMode)
            {
                case UVMode.Clip: startV = (float)GetSample(sampleCount - 1).percent; break;
                case UVMode.UniformClip: startV = spline.CalculateLength(0.0, GetSample(sampleCount - 1).percent); break;
            ...
            for (int lat = 1; lat < _roundCapLatitude+1; lat++)
            {
                ...
                for (int lon = 0; lon <= sides; lon++)
                {
                    ...
                    Quaternion rot = Quaternion.AngleAxis(_revolve * anglePercent + rotation + 180f, Vector3.forward) * Quaternion.AngleAxis(latAngle, -Vector3.up);
                    tsMesh.vertices[index] = center + lookRot * rot * Vector3.right * size * 0.5f * GetSample(sampleCount - 1).size;
                    tsMesh.normals[index] = (tsMesh.vertices[index] - center).normalized;
                    tsMesh.colors[index] = GetSample(sampleCount - 1).color * color;
                    tsMesh.uv[index] = new Vector2(anglePercent*uvScale.x, (startV + capLengthPercent * latitudePercent)*uvScale.y) - uvOffset;
                }
            }
