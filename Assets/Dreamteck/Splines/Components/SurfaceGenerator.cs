...
namespace Dreamteck.Splines
{
    ...
    public class SurfaceGenerator : MeshGenerator
    {
        ...
        {
            if (_extrudeSpline != null) _extrudeSpline.Evaluate(ref extrudeResults, _extrudeFrom, _extrudeTo);
            int surfaceVertexCount = sampleCount;
            if (spline.isClosed) surfaceVertexCount--;
            int vertexCount = surfaceVertexCount;
            bool pathExtrude = _extrudeSpline != null && extrudeResults.Length > 0;
            ...
            {
                tsMesh.vertices[i] = GetSample(i).position + off;
                tsMesh.normals[i] = GetSample(i).normal;
                tsMesh.colors[i] = GetSample(i).color;
            ...
            if (_expand != 0f)
            {
                for (int i = 0; i < surfaceVertexCount; i++)
                {
                    ModifySample(GetSample(i), evalResult);
            ...
            if (pathExtrude)
            {
                ...
                for (int i = 0; i < surfaceVertexCount; i++)
                {
                    ...
                    tsMesh.normals[i + surfaceVertexCount] = -extrudeResults[0].direction;
                ...
                written = WriteTris(ref wallTris, ref tsMesh.triangles, surfaceVertexCount * 2, written, false);
            }
            else if (simpleExtrude)
            {
                ...
                {
                    int index = i;
                    if (i == surfaceVertexCount) index = 0;
                    tsMesh.vertices[i + surfaceVertexCount * 2] = tsMesh.vertices[index] - normal * _extrude;
                    tsMesh.normals[i + surfaceVertexCount * 2] = clockwise ? -GetSample(index).right : GetSample(index).right;
                    ...
                    tsMesh.vertices[i + surfaceVertexCount * 3 + 1] = tsMesh.vertices[index] ;
                    tsMesh.normals[i + surfaceVertexCount * 3 + 1] = tsMesh.normals[i + surfaceVertexCount * 2];
                    tsMesh.colors[i + surfaceVertexCount * 3 + 1] = tsMesh.colors[index];
                    if (_uniformUvs) tsMesh.uv[i + surfaceVertexCount * 3 + 1] = new Vector2((float)i / surfaceVertexCount * _sideUvScale.x + _sideUvOffset.x, _extrude * _sideUvScale.y + _sideUvOffset.y);
                    else tsMesh.uv[i + surfaceVertexCount * 3 + 1] = new Vector2((float)i / surfaceVertexCount * _sideUvScale.x + _sideUvOffset.x, 1f * _sideUvScale.y + _sideUvOffset.y);
                    if (clockwise) tsMesh.uv[i + surfaceVertexCount * 3 + 1].x = 1f - tsMesh.uv[i + surfaceVertexCount * 3 + 1].x;
                }
            ...
            {
                //for (int i = 0; i < tsMesh.vertices.Length; i++) tsMesh.vertices[i] += clockwise ? -GetSample(i).right : GetSample(i).right;
        ...
        void GetIdentityVerts(Vector3 center, Vector3 normal, bool clockwise)
        {
            ...
            {
                identityVertices[i] = vertsRotation * (GetSample(i).position - center + (clockwise ? -GetSample(i).right : GetSample(i).right) * _expand);
                identityNormals[i] = vertsRotation * (clockwise ? -GetSample(i).right : GetSample(i).right);
            }
        }
        ...
        void GetProjectedVertices(int count, out Vector3 center, out Vector3 normal)
        {
            ...
            {
                center += GetSample(i).position + off;
                normal += GetSample(i).normal;
            }
            normal.Normalize();
            center /= count;
            ...
            for (int i = 0; i < count; i++)
            {
                Vector3 point = GetSample(i).position + off - center;
